...
! *****************************************************************************     
      ...
      SUBROUTINE GET_VARIABLES()
       ...
       DHDTVar => VariableGet(Solver%Mesh%Variables,'dhdt')
       IF (.NOT.ASSOCIATED(DHDTVar)) &
         DHDTVar => VariableGet(Solver%Mesh%Variables,'H Velocity')
       IF (.NOT.ASSOCIATED(DHDTVar)) &
         CALL FATAL(SolverName,'DHDT not Found')
      ...
      REAL(KIND=dp),INTENT(OUT) :: Volume,VAF,&
                       ...
                       GroundedArea,FloatingArea,FreeArea
      REAL(KIND=dp),parameter :: Fsmall=EPSILON(1.0)
      ...
      DO t = 1,ne
         ...
         MinH=0._dp
         MinH(1:n) = ListGetReal(BodyForce,'H Lower Limit',n,NodeIndexes,Found )
         IF (.NOT.Found) &
            MinH(1:n) = ListGetReal(Material,'Min H',n,NodeIndexes,Found )
         IF (.NOT.Found) CALL FATAL(SolverName,"Min H not found")
